...
int main() {
    ...
    while (t--) {
       ...
       cin>>i;
        string str;
        cin >> str; // Read the string for the current test case

        // Check if the first and last characters of the string are the same
        if (str[0] == str[str.length() - 1]) {
            cout << "NO" << endl; // Output "NO" if the first and last characters are the same
        } else {
            cout << "YES" << endl; // Output "YES" if the first and last characters are different
        }
    }
    return 0;
}